...
namespace BackEndSAM.Controllers
{
    ...
    public class IncidenciaController : ApiController
    {
        ...
        [HttpGet]
        public object GetRequiereIncidenciaBilingue(int TipoIncidenciaID, int RegistroID, string token)
        {
            string payload = "";
            string newToken = "";
            bool tokenValido = ManageTokens.Instance.ValidateToken(token, out payload, out newToken);
            if (tokenValido)
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                Sam3_Usuario usuario = serializer.Deserialize<Sam3_Usuario>(payload);
                return IncidenciaBd.Instance.IncidenciaBilingue(TipoIncidenciaID,RegistroID);
            }
            else
            {
                TransactionalInformation result = new TransactionalInformation();
