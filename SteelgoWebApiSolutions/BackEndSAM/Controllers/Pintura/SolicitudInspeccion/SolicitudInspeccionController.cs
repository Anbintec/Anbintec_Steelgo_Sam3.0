using BackEndSAM.DataAcces.Pintura.SolicitudInspeccion;
...
namespace BackEndSAM.Controllers.Pintura.SolicitudInspeccion
{
    [EnableCors(origins: "*", headers: "*", methods: "*")]
    public class SolicitudInspeccionController : ApiController
    {
        ...
        public object GetSpoolIDRequisitado(string token, string OrdenTrabajo, string Lenguaje)
        {
            try
            {
                ...
                if (tokenValido)
                {
                    ...
                    List<Sam3_Pintura_GET_SpoolIDRequisitado_Result> lista = (List<Sam3_Pintura_GET_SpoolIDRequisitado_Result>)SolicitudInspeccionBD.Instance.ObtenerSpoolID(OrdenTrabajo, Lenguaje);
        ...
        [HttpPost]
        public object GuardaCaptura(Datos ListaElementos, string Lenguaje, string token)
        {
            try
            {
                string payLoad = "";
                ...
                if (tokenValido)
                {
                    ...
                    return SolicitudInspeccionBD.Instance.GuardarCaptura(Usuario, dtListaDatos);
                }
                else
                {
                    TransactionalInformation result = new TransactionalInformation();
        ...
        public object GetInfoBySpool(string NumeroControl, string token)
        {
            try
            {
                ...
                if (tokenValido)
                {
                    return SolicitudInspeccionBD.Instance.ObtenerInfoPorSpool(NumeroControl);
                }
                else
                {
                    TransactionalInformation result = new TransactionalInformation();
