...
namespace BackEndSAM.Controllers
{
    ...
    public class ItemCodeController : ApiController
    {
        ...
        public object Get(int TipoPackingListID, string token, int paginaID, string idioma, int proyectoID,string datosBuscar)
        {
            string payload = "";
            string newToken = "";
            ...
            if (tokenValido)
            {
                ...
                return ItemCodeBd.Instance.ObtenerItemCode(TipoPackingListID, usuario, paginaID, idioma, proyectoID, datosBuscar);
            }
            else
            {
        ...
        public object Get(string itemCodeID, string token, string Colada, string MM, int Cantidad)
        {
            string payload = "";
            string newToken = "";
            bool tokenValido = ManageTokens.Instance.ValidateToken(token, out payload, out newToken);
            if (tokenValido)
            {
                return ItemCodeBd.Instance.ObtenerDetalleItemCode(itemCodeID, Colada, MM, Cantidad);
            }
            else
            {
                TransactionalInformation result = new TransactionalInformation();
