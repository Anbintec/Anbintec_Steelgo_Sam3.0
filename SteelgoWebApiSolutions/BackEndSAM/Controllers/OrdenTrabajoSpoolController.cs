    ...
    public class OrdenTrabajoSpoolController : ApiController
    {
        public object Get(string Busqueda, string token)
        {
            ...
            if (tokenValido)
            {
                ...
                return OrdenTrabajoSpoolBd.Instance.ListadoNumerosDeControlPreDespachoYDespacho(Busqueda, usuario);
            }
            else
            {
        ...
        public object Get(string numeroControl, string token, bool etiquetas = false)
        {
            string payload = "";
            string newToken = "";
            bool tokenValido = ManageTokens.Instance.ValidateToken(token, out payload, out newToken);
            if (tokenValido)
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                Sam3_Usuario usuario = serializer.Deserialize<Sam3_Usuario>(payload);
                return OrdenTrabajoSpoolBd.Instance.EtiquetasDisponibles(numeroControl, usuario);
            }
            else
            {
                TransactionalInformation result = new TransactionalInformation();
