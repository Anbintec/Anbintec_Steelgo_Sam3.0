...
function AjaxObtenerInfoBySpool(numControl) {
    try {
        loadingStart();        
        $RequisicionInspeccion.RequisicionInspeccion.read({ NumeroControl: numControl, token: Cookies.get("token") }).done(function (data) {
            if (Error(data)) {
...
function AjaxObtenerDatosGrid(opc) {
    loadingStart();
    try {
        $RequisicionInspeccion.RequisicionInspeccion.read({token: Cookies.get("token"), Opc: opc, Relleno: true}).done(function (data) {
            if (Error(data)) {
...
function AjaxGuardarCaptura(data, esGuardarNuevo, esEliminacion) {
    try {
        loadingStart();
        var Captura = { Detalle: "" };
        var listaDetalle = [];
        for (var i = 0; i < data.length; i++) {
            ...
            listaDetalle[i].FechaSolicitud = (data[i].FechaSolicitud == "" || data[i].FechaSolicitud == null) ? null : ConvertirDateTime(data[i].FechaSolicitud);
        ...
        $RequisicionInspeccion.RequisicionInspeccion.create(Captura, { token: Cookies.get("token"), Param: 0 }).done(function (data) {
...
function ConvertirDateTime(value) {
    ...
    return value.getDate() + "/" + (value.getMonth() + 1) + "/" + value.getFullYear() + " " + HH + ":" + mm + ":" + ss;
}
